    ...
    \resizebox{\columnwidth}{!}{%
      \begin{tikzpicture}
        ...
        \path
            ...
            to ++(0:1.15 * \widthepoch) node[block, fill=mydarkpurple,
        ...
        \draw[->, thick] (S0B-right) -- node[below, align=center]{\emph{insert "L"}\\\emph{between}\\\emph{"E" and "L"}} (S1B-left);
        \draw[dashed, ->, thick, shorten >= 3] (S1B-right.east) -- node[right, xshift=5pt, align=center]{\emph{insert "L" at} {\color{mylightorange}\id{i}{B0}{0}\id{m}{B1}{0}} $\to$ {\color{mylightblue!20!mydarkblueid}\id{i}{A1}{1}\id{i}{B0}{0}\id{m}{B1}{0}}} (S2A-left.west);
...
\begin{frame}{How to do so ?}
  \begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[thick, ->] (e0) -- (eA1);
        \draw[thick, ->] (e0) -- (eB2);
        \draw[->, dashed, thick, red] (eB2.180) -- (eA1.0) (eA1.25) -- (e0.270);
    \end{tikzpicture}
    \caption{\emph{Epoch tree} corresponding to previous scenario}
  \end{figure}
  \vspace{-3mm}
  \begin{itemize}
    \item<1-> Define total order on epochs to select target epoch
    \item<2> Design transformation function to revert \emph{rename} operation
  \end{itemize}
\end{frame}
...
\begin{frame}{What about concurrent \emph{rename} operations ?}
  \begin{figure}
    ...
    \begin{tikzpicture}
        \path
            node {\textbf{A}}
            to ++(0:0.5 * \widthletter) node[epoch] (S2A-left) {\epoch{A1}}
            to ++(0:1.15 * \widthepoch) node[block, fill=mydarkblue,
                    label={below:{\color{mydarkblueid}\id{i}{A1}{0..1}} }
                        ] {HE}
            to ++(0:\widthblock) node[letter, fill=mylightblue,
        ...
        \path
            to ++(270:3) node {\textbf{B}}
            to ++(0:0.5 * \widthletter) node[epoch] (S2B-left) {\epoch{B2}}
            to ++(0:1.15 * \widthepoch) node[block, fill=mydarkpurple,
                label={ [] below:{\color{mydarkpurpleid}\id{i}{B2}{0..4}} }
            ] (S2B-right) {HELLO};
  ...
  \vspace{-3mm}
  \begin{itemize}
    \item Revert state to equivalent one at LCA epoch
    \item Apply then \emph{rename} operations leading to target epoch
  \end{itemize}
\end{frame}
